		...
		'tinymce.dom.DOMUtils' : {
			...
			*/
			...
			select : function(pattern, scope) {
				...
				return $.find(pattern, t.get(scope) || t.get(t.cfg.root_element) || t.doc, []);
			},
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.DOMUtils', {
		...
		getAttrib : function(e, n, dv) {
			...
			if (n === 'style') {
				...
				if (v) {
					...
					if (t.cfg.keep_values && !t._isRes(v))
						e.setAttribute('_mce_style', v);
				}
			}
		...
		getClasses : function() {
			var t = this, cl = [], i, lo = {}, f = t.cfg.class_filter, ov;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Serializer', {
		...
		_postProcess : function(o) {
			var t = this, s = t.cfg, h = o.content, sc = [], p;
...
/*				if (o.set)
		...
		_serializeNode : function(n, inner) {
			var t = this, s = t.cfg, w = t.writer, hc, el, cn, i, l, a, at, no, v, nn, ru, ar, iv, closed, keep, type, scopeName;
	...
	tinymce.create('tinymce.ui.Control', {
		...
		setDisabled : function(s) {
			...
			if (s != this.disabled) {
				...
				if (e && this.cfg.unavailable_prefix) {
					if (s) {
						this.prevTitle = e.title;
						e.title = this.cfg.unavailable_prefix + ": " + e.title;
					} else
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.DropMenu:tinymce.ui.Menu', {
		...
		update : function() {
			var t = this, s = t.cfg, tb = DOM.get('menu_' + t.id + '_tbl'), co = DOM.get('menu_' + t.id + '_co'), tw, th;
		...
		showMenu : function(x, y, px) {
			var t = this, s = t.cfg, co, vp = DOM.getViewPort(), w, h, mx, my, ot = 2, dm, tb, cp = t.classPrefix;
			...
			if (!t.rendered) {
				co = DOM.add(t.cfg.container, t.renderNode());
...
//			n = DOM.add(co, 'div', {id : 'menu_' + t.id + '_co', 'class' : 'mceMenuContainer'});
		...
		_add : function(tb, o) {
			var n, s = o.cfg, a, ro, it, cp = this.classPrefix, ic;
...
//			n = DOM.add(n, 'span', {'class' : 'item'});
			...
			n = DOM.add(n, s.element || 'span', {'class' : 'mceText', title : o.cfg.title}, o.cfg.title);

			if (o.cfg.style)
				DOM.setAttrib(n, 'style', o.cfg.style);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.Button:tinymce.ui.Control', {
		...
		renderHTML : function() {
			var cp = this.classPrefix, s = this.cfg, h, l;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.ListBox:tinymce.ui.Control', {
		...
		selectByIndex : function(idx) {
			...
			if (idx != t.selectedIndex) {
				...
				if (o) {
					...
					DOM.setHTML(e, DOM.encode(t.cfg.title));
		...
		renderHTML : function() {
			var h = '', t = this, s = t.cfg, cp = t.classPrefix;

			h = '<table id="' + t.id + '" cellpadding="0" cellspacing="0" class="' + cp + ' ' + cp + 'Enabled' + (s['class'] ? (' ' + s['class']) : '') + '"><tbody><tr>';
			h += '<td>' + DOM.createHTML('a', {id : t.id + '_text', href : 'javascript:;', 'class' : 'mceText', onclick : "return false;", onmousedown : 'return false;'}, DOM.encode(t.cfg.title)) + '</td>';
		...
		showMenu : function() {
			...
			p1 = DOM.getPos(this.cfg.menu_container);
			p2 = DOM.getPos(e);

			m = t.menu;
			m.cfg.offset_x = p2.x;
			m.cfg.offset_y = p2.y;
			m.cfg.keyboard_focus = !tinymce.isOpera; // Opera is buggy when it comes to auto focus
		...
		renderMenu : function() {
			...
			m = t.cfg.control_manager.createDropMenu(t.id + '_menu', {
				menu_line : 1,
			...
			each(t.items, function(o) {
				...
				if (o.value === undefined) {
					...
					o.id = DOM.uniqueId();
					o.onclick = function() {
						if (t.cfg.onselect(o.value) !== false)
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.NativeListBox:tinymce.ui.ListBox', {
		...
		renderHTML : function() {
			...
			h = DOM.createHTML('option', {value : ''}, '-- ' + t.cfg.title + ' --');
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.MenuButton:tinymce.ui.Button', {
		...
		showMenu : function() {
			...
			p1 = DOM.getPos(t.cfg.menu_container);
			p2 = DOM.getPos(e);

			m = t.menu;
			m.cfg.offset_x = p2.x;
			m.cfg.offset_y = p2.y;
			m.cfg.vp_offset_x = p2.x;
			m.cfg.vp_offset_y = p2.y;
			m.cfg.keyboard_focus = t._focused;
			m.showMenu(0, e.clientHeight);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton', {
		...
		renderMenu : function() {
			var t = this, m, i = 0, s = t.cfg, n, tb, tr, w;
...
tinymce.create('tinymce.ui.Toolbar:tinymce.ui.Container', {
	renderHTML : function() {
		var t = this, h = '', c, co, dom = tinymce.DOM, s = t.cfg, i, pr, nx, cl;
		...
		return dom.createHTML('table', {id : t.id, 'class' : 'mceToolbar' + (s['class'] ? ' ' + s['class'] : ''), cellpadding : '0', cellspacing : '0', align : t.cfg.align || ''}, '<tbody><tr>' + h + '</tr></tbody>');
	}
});
	...
	tinymce.EditorManager = extend(tinymce, {
		...
		execCommand : function(c, u, v) {
			...
			switch (c) {
				...
				case "mceAddControl":
					if (!t.get(v))
						new tinymce.Editor(v, t.cfg).render();
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			var t = this, s = t.cfg, id = t.id, sl = tinymce.ScriptLoader;
		...
		init : function() {
			var n, t = this, s = t.cfg, w, h, e = t.getElement(), o, ti, u, bi, bc, re;
		...
		setupIframe : function() {
			var t = this, s = t.cfg, e = DOM.get(t.id), d = t.getDoc(), h, b;
			...
			t.formatter.register(t.cfg.formats);
		...
		focus : function(sf) {
			var oed, t = this, ce = t.cfg.content_editable, ieRng, controlElm, doc = t.getDoc();
		...
		translate : function(s) {
			var c = this.cfg.language || 'en', i18n = tinymce.i18n;
		...
		getLang : function(n, dv) {
			return tinymce.i18n[(this.cfg.language || 'en') + '.' + n] || (is(dv) ? dv : '{#' + n + '}');
		},

		getParam : function(n, dv, ty) {
			var tr = tinymce.trim, v = is(this.cfg[n]) ? this.cfg[n] : dv, o;
		...
		getElement : function() {
			return DOM.get(this.cfg.content_element || this.id);
		},
		...
		destroy : function(s) {
			...
			t.contentAreaContainer = t.formElement = t.container = t.cfg.content_element = t.bodyElement = t.contentDocument = t.contentWindow = null;
		...
		_addEvents : function() {
			...
			var t = this, i, s = t.cfg, dom = t.dom, lo = {
				mouseup : 'onMouseUp',
...
(function(tinymce) {
	...
	tinymce.UndoManager = function(editor) {
		...
		return self = {
			...
			add : function(level) {
				var i, settings = editor.cfg, lastLevel;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ForceBlocks', {
		...
		setup : function() {
			var t = this, ed = t.editor, s = ed.cfg, dom = ed.dom, selection = ed.selection;
		...
		forceRoots : function(ed, e) {
				...
				if (nx.nodeType === 3 || (!t.dom.isBlock(nx) && nx.nodeType !== 8 && !/^(script|mce:script|style|mce:style)$/i.test(nx.nodeName))) {
					if (!bl) {
						...
						if (nx.nodeType != 3 || /[^\s]/g.test(nx.nodeValue)) {
							...
							// See: http://support.microsoft.com/kb/829907
							bl = ed.dom.create(ed.cfg.forced_root_block);
			...
			if (si != -2) {
				if (!isIE || r.setStart) {
					bl = b.getElementsByTagName(ed.cfg.element)[0];
					r = d.createRange();
		...
		insertPara : function(e) {
			var t = this, ed = t.editor, dom = ed.dom, d = ed.getDoc(), se = ed.cfg, s = ed.selection.getSel(), r = s.getRangeAt(0), b = d.body;
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ControlManager', {
		...
		createDropMenu : function(id, s, cc) {
			...
			s = extend({
				'class' : 'mceDropDown',
				constrain : ed.cfg.constrain_menus
			}, s);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.WindowManager', {
		...
		open : function(s, p) {
			var t = this, f = '', x, y, mo = t.editor.cfg.dialog_type == 'modal', w, sw, sh, vp = tinymce.DOM.getViewPort(), u;
...
(function(tinymce) {
	tinymce.Formatter = function(ed) {
		var formats = {},
			...
			isBlock = dom.isBlock,
			forcedRootBlock = ed.cfg.forced_root_block,
			nodeIndex = dom.nodeIndex,
			INVISIBLE_CHAR = '\uFEFF',
...
tinymce.onAddEditor.add(function(tinymce, ed) {
	var filters, fontSizes, dom, settings = ed.cfg;
